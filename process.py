...
def create_simulated_fasta(input_file, sim_output_dir, sequence_length, kmer_length=3, randomness=0.0, seed=None):
    ...
    filename = os.path.join(sim_output_dir, os.path.basename(input_file).replace('.fasta', '_sim.fasta'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sim_output_dir', type=str, required=True, help='Directory to save simulated fasta files')
    ...
    parser.add_argument('--sim_size_kb', type=int, default=1000, help='Size of the simulated file in kilobases')
    ...
    parser.add_argument('--skip_subsample', action='store_true', help='Skip subsampling step if specified')
    args = parser.parse_args()

    sequence_length = args.sim_size_kb * 1000  # Convert kilobases to bases
    ...
    for file in os.listdir(args.output_dir):
        if file.endswith('.fasta'):
            ...
            create_simulated_fasta(input_file, args.sim_output_dir, sequence_length, args.kmer_length, args.randomness, args.seed)
